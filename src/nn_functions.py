...
def hessian_eigs(params, coords, target):
  h = hessian(loss)(params, coords, target)
  eigvals = jnp.linalg.eigvalsh(h)
  return eigvals

def make_hvp_fn(params, coord, target):
    flat_params, unravel_fn = ravel_pytree(params)
    flat_loss = lambda p: loss(unravel_fn(p), coord, target)
    
    def hvp(v):
        grad_fn = jax.grad(flat_loss)
        _, hv = jax.jvp(grad_fn, (flat_params,), (v,))
        return np.array(hv)

    return hvp, flat_params.shape[0]

def hessian_eigenvals(params, coord, target, k=20):
    hvp, dim = make_hvp_fn(params, coord, target)
    linop = LinearOperator((dim, dim), matvec=hvp, dtype=np.float32)
    eigvals = eigsh(linop, k=k, return_eigenvectors=False)
    return eigvals
